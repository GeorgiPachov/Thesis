...
public class AdjectiveAdjectiveNoun implements PhraseFormula {
	...
	for (int i = 0; i < posTokens.size() - 3; i++) {
	    ...
	    if (current.equals(PosType.ADJECTIVE) && next.equals(PosType.ADJECTIVE) && afterNext.equals(PosType.NOUN)) {
		phrases.add(new Phrase(posTokens.get(i), posTokens.get(i + 1), posTokens.get(i+2)));
	    }
	}
	return phrases;
    }
